...
def main(argv: list[str] | None = None) -> int:
    ...
    if run_tqa and run_triage:
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            fut_tqa: Future[tuple[bool, Path, Path]] = pool.submit(
                stage_tqa,
                python=args.python,
            ...
            fut_triage: Future[tuple[bool, Path]] = pool.submit(
                stage_triage,
                python=args.python,
        ...
        # Run sequentially (only one or zero stages)
        if run_tqa:
            stages_run.append("tqa")
            tqa_ok, tqa_json_path, tqa_md_path = stage_tqa(
                python=args.python,
                tqa_script=tqa_script,
                root=root,
                out_dir=out_dir,
        ...
        if run_triage:
            ...
            triage_ok, triage_dir = stage_triage(
                python=args.python,
                triage_script=triage_script,
                root=root,
                suites=args.suite,
