...
STDLIB: frozenset[str] = frozenset(_sys.stdlib_module_names)
...
def _norm(name: str) -> str:
    """Normalise a package/dist name for case-insensitive comparison."""
    return name.lower().replace("_", "-")


def _dist_candidates(module: str) -> tuple[str, str]:
    """(dist name, confidence).  Exact table hit → 'high', else guess → 'medium'."""
...
def _spec_name(spec: str) -> str:
    """Extract the normalised name from a PEP 508 dependency specifier."""
...
def _is_test_scope(rel: str) -> bool:
    """True when *rel* lives under a test dir or is named test_*."""
    return "tests" in Path(rel).parts or Path(rel).name.startswith("test_")


def _walk_top_imports(tree: ast.AST) -> list[tuple[str, int]]:
    """Return [(top_level_module, lineno)] for every top-level import in *tree*."""
...
def collect_imports(
    ...
    """Return {top-level-module: [(relpath, lineno, is_test_scope)]}."""
...
def _parse_pyproject_deps(root: Path) -> tuple[list[str], bool]:
    """Return (list-of-spec-strings, found?) from root/pyproject.toml."""
...
def _is_local_module(root: Path, module: str, source_prefixes: list[str]) -> bool:
    """*module* is local when root/<module>.py, root/<module>/__init__.py, or
    any <prefix>/<module>/ exists."""
...
def _load_advisory(path_str: str) -> list[dict]:
    """Load and validate the C-8 advisory report JSON.  Raises ToolError."""
    try:
...
def _map_advisory_severity(raw_sevs: set) -> tuple[str, str]:
    ...
    for s in raw_sevs:
        if s is None:
            ...
            mapped.append("medium")
    best = max(mapped, key=lambda x: _SEVERITY_ORDER.get(x, 0))
    return (best, "medium" if has_null else "high")


# ---------------------------------------------------------------------------
# Finding builders (decomposed from the old monolithic analyze_tree)
# ---------------------------------------------------------------------------
...
def _find_unused_declared(
    ...
    """Emit DELETE findings for declared deps never imported."""
...
def _build_advisory_vuln(
    ...
    fix_info = ", ".join(
        f"{v['id']} (fix: {', '.join(v.get('fix_versions', []))})" for v in vulns
    )
    count = len(vulns)
...
def _find_advisory_findings(
    ...
    for pkg in packages:
        ...
        if vulns:
            ...
            findings.append(_build_advisory_vuln(norm_name, pkg_path, vulns, sev, conf))
...
def analyze_tree(
    ...
    thresholds: dict,  # noqa: ARG001  reserved for future use
    ...
    findings: list[hc.Finding] = []
    findings.extend(_find_unused_declared(declared_dists, imported_dists, declared))
    findings.extend(
    ...
    if advisory_report:
        findings.extend(_find_advisory_findings(advisory_report, declared, root))

    return hc.sort_findings(findings)


# ---------------------------------------------------------------------------
# Report rendering  (uses get() instead of setdefault() to reduce duplication)
# ---------------------------------------------------------------------------
...
def load_thresholds(config_path: str | None) -> dict[str, float]:
    """Merge overrides from an optional JSON file into defaults."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--config", help="JSON file overriding thresholds.")
...
def _validate_required_args(args) -> bool:
    """Return False (and print error) when --root or --out-dir is missing."""
    needed = not (args.root and args.out_dir)
    if needed:
        print(
            json.dumps(
                {
                    "status": "error",
                    "message": "--root and --out-dir are required",
                }
            )
        )
    return not needed
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not _validate_required_args(args):
